    ...
    def test_tool_name_consistency(self):
        ...
        for tool_list in [graph_tools, data_tools, eval_tools, supervisor_tools]:
            for tool in tool_list:
                ...
                # 检查原始函数名称
                if hasattr(tool, '_original') and hasattr(tool._original, '__name__'):
                    original_name = tool._original.__name__
                else:
                    original_name = "未知"
                print(f"工具: {tool.name} (原始函数名称: {original_name})")
        ...
        for tool_name in prompted_tool_names:
            ...
            if tool_name not in actual_tool_names:
                ...
                print(f"缺失工具: {tool_name}")
        
        if missing_tools:
            self.fail(f"以下工具在提示词中提到但实际未实现: {missing_tools}")
