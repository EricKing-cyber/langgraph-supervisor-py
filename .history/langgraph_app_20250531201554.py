...
def handle_communication(state: Dict[str, Any]) -> Dict[str, Any]:
    """处理监督者之间的通信"""
    messages = state.get("messages", [])
    if not messages:
        return {"next": "__end__"}
    ...
    # 获取通信状态
    communication_state = state.get("communication_state", {})
    last_message = messages[-1]
    
    # 检查是否需要团队协作
    if "需要协作" in last_message.content or "complex" in last_message.content.lower():
        # 添加协作请求消息
        messages.append(AIMessage(content="正在协调团队协作..."))
        communication_state["needs_collaboration"] = True
        communication_state["collaborating_teams"] = ["math_team", "weather_team"]
    
    # 检查任务状态
    if "task_status" in communication_state:
        status = communication_state["task_status"]
        if status == "in_progress":
            messages.append(AIMessage(content="任务正在处理中..."))
        elif status == "completed":
            messages.append(AIMessage(content="任务已完成"))
...
app.add_node("communication_handler", handle_communication)

# 设置边
app.add_edge("__start__", "task_router")
app.add_edge("task_router", "communication_handler")
app.add_edge("communication_handler", "supervisor")
