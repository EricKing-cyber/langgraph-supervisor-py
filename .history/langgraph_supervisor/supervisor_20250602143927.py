...
def create_top_level_supervisor(
    ...
    return builder
def create_supervisor(
    agents: list[Union[StateGraph, Pregel]],
    model: LanguageModelLike,
    ...
):
    # 改为只检查是否提供了 name 参数，而不是访问 agent.name
    for agent in agents:
        if not hasattr(agent, "name") or agent.name is None or agent.name == "LangGraph":
            raise ValueError("请为你的代理指定一个唯一名称")