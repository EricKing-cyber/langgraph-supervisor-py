...
def create_top_level_supervisor(
    ...
    # 编译中间层监督者图
    compiled_supervisors = [sv.compile(name=f"{supervisor_name}_sub_{i}") for i, sv in enumerate(middle_supervisors)]

    # 创建基础监督者
    base_supervisor = create_supervisor(
        agents=compiled_supervisors,
        model=model,
        tools=tools,
        prompt=prompt,
        response_format=response_format,
    ...
    # 添加中间层监督者节点
    for i, sv in enumerate(compiled_supervisors):
