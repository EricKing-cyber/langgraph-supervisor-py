...
from typing import Dict, Any, List, Union, Optional, Sequence, Self
...
# 扩展StateGraph类以支持set_finish_point方法
class ExtendedStateGraph(StateGraph):
    def set_finish_point(self, key: str) -> Self:
        ...
        if key == START:
            raise ValueError("START cannot be a finish point")
        if key not in self.nodes and key != END:
            raise ValueError(f"Need to add_node `{key}` first")
            
        return self.add_edge(key, END)
        
# 创建应用
app = ExtendedStateGraph(state_schema=CustomAgentState)  # 使用增强型状态模型
...
# 正确设置完成点
app.set_finish_point("supervisor")
