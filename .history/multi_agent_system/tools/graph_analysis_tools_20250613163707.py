...
create_knowledge_graph = tool_wrapper(_create_entity_graph)
add_entities_to_graph = _create_entity_graph  # 添加别名
analyze_graph_centrality = tool_wrapper(_analyze_graph_centrality)
find_path_between_entities = tool_wrapper(_find_shortest_path)  # 确保与提示词中的名称一致
detect_communities = tool_wrapper(_detect_communities)
visualize_graph = tool_wrapper(_visualize_graph)
graph_similarity_analysis = tool_wrapper(_graph_similarity_analysis)
extract_subgraph = tool_wrapper(_extract_subgraph)

# 添加别名 - 确保名称与提示词中的一致
add_relationships_to_graph = _create_entity_graph  # 添加别名
list_knowledge_graphs = lambda: [str(p) for p in GRAPH_DATA_DIR.glob("*.json")]  # 简单实现列表功能
...
def get_graph_analysis_tools() -> List[BaseTool]:
    ...
    """
    # 为别名创建工具
    add_entities_to_graph_tool = tool_wrapper(add_entities_to_graph)
    add_relationships_to_graph_tool = tool_wrapper(add_relationships_to_graph)
    list_knowledge_graphs_tool = tool(list_knowledge_graphs)
