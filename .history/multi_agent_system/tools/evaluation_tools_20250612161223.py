...
@tool
def evaluate_report_quality(report_content: str, criteria: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    评估研究报告的质量
...
@tool
def evaluate_reliability(text: str, check_sources: bool = True) -> Dict[str, Any]:
    """
    评估文本内容的可靠性和真实性
...
@tool
def evaluate_content_relevance(content: str, topic: str, keywords: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    评估内容与主题的相关性
    ...
    if not keywords:
        ...
        try:
            ...
            common_words = {'the', 'and', 'of', 'to', 'a', 'in', 'for', 'is', 'on', 'that', 'by', 'this', 'with', 'are'}
...
@tool
def evaluate_coherence(text: str) -> Dict[str, Any]:
    """
    评估文本的连贯性和逻辑一致性
...
# 应用包装器到各个工具
evaluate_report_quality = direct_tool_wrapper(evaluate_report_quality)
evaluate_reliability = direct_tool_wrapper(evaluate_reliability)
evaluate_content_relevance = direct_tool_wrapper(evaluate_content_relevance)
evaluate_coherence = direct_tool_wrapper(evaluate_coherence)
