...
def _visualize_graph(graph_file: str, layout: str = "spring", output_format: str = "png") -> str:
    ...
    # 尝试几种常见的中文字体
    chinese_fonts = ['SimHei', 'Microsoft YaHei', 'SimSun', 'FangSong', 'KaiTi', 'WenQuanYi Micro Hei']
    font_found = False
    ...
    # 如果没找到中文字体，使用系统默认字体并发出警告
    if not font_found:
        print("警告: 未找到支持中文的字体，可能导致部分文字显示异常")
    ...
    # 选择颜色方案（默认使用bold配色）
    color_scheme = color_palettes["bold"]
    ...
    if important_nodes:
        nx.draw_networkx_labels(
            G, pos, 
            labels={node: node_labels[node] for node in important_nodes},
            font_size=10,
            font_weight='bold',
    ...
    plt.savefig(output_path, format=output_format, bbox_inches="tight", dpi=300, facecolor='#f9f9f9')
    plt.close()
...
@tool
def visualize_graph(graph_file: str, layout: str = "spring") -> str:
    """
    可视化图谱
    ...
    Returns:
        str: 图像文件路径
    """
    return _visualize_graph(graph_file, layout)
