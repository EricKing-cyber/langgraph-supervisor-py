from langgraph_supervisor.supervisor import create_supervisor,create_top_level_supervisor, NamedStateGraph
...
def build_top_level_supervisor(middle_supervisors_with_names, model):
    """
    ...
    Args:
        ...
        model: 语言模型
    """
    named_middle_supervisors = [
        NamedStateGraph(graph=sg, name=name) for sg, name in middle_supervisors_with_names
    ]

    return create_top_level_supervisor(
        middle_supervisors=named_middle_supervisors,
        model=model,
    )
