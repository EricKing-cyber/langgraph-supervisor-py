...
def build_supervisor_workflow(agents, model):
    ...
    # 定义代理节点处理函数
    def agent_node(state: Dict[str, Any], agent_name: str) -> Dict[str, Any]:
        # 处理代理的响应
        messages = state.get("messages", [])
        if not messages:
            return {"next": "__end__"}
        ...
        # 添加代理的响应
        response = f"{agent_name} 已处理任务"
        return {
            "next": "base_supervisor",
    ...
    for agent in agents:
        workflow.add_node(
            agent.name,
            lambda state, name=agent.name: agent_node(state, name)
        )
