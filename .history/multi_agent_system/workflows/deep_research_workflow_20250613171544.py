...
def get_supervisor_tools() -> list[BaseTool]:
    ...
    # 检查和调试工具注册
    tool_names = []
    for tool_list in [base_tools, evaluation_tools, persistence_tools, graph_analysis_tools]:
        for t in tool_list:
...
def build_deep_research_workflow(research_agents: list, model: LanguageModelLike, config: Dict[str, Any] = None):
    ...
    【重要：并行任务处理】
    你可以同时将多个任务分配给不同的研究员，而不需要等待一个研究员完成工作后再分配下一个任务。
    例如，当你完成报告结构规划后，可以同时调用多个transfer_to_X_researcher工具，将不同章节分配给相应专家。
    并行处理可以显著提高研究效率，加快报告生成速度。
    ...
    if 'configurable' in config and 'parallel_tool_calls' in config['configurable'] and config['configurable']['parallel_tool_calls']:
    ...
    graph = create_supervisor(
        ...
        parallel_tool_calls=parallel_tool_calls  # 使用langgraph_supervisor内置的parallel_tool_calls参数
    )
