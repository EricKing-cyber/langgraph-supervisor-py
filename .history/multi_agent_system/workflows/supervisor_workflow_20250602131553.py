...
def build_top_level_supervisor(middle_supervisors, model):
    ...
    Args:
        ...
        model: 语言模型
    """    
    compiled_supervisors = [
        (sg.compile(name=name), name) for sg, name in middle_supervisors       #生成器表达式（generator expression）
    ]
    
    return create_top_level_supervisor(
        middle_supervisors=[sg for sg, name in compiled_supervisors],
        model=model,
    )
