...
def build_supervisor_workflow(agents, model):
    ...
    def supervisor_node(state: Dict[str, Any]) -> Dict[str, Any]:
        # 简单的任务分发逻辑
        if "math" in state.get("messages", [])[-1].content.lower():
            return {"next": "math_expert"}
        elif "weather" in state.get("messages", [])[-1].content.lower():
    ...
    # 设置边
    workflow.add_edge("__start__", "supervisor")  # 使用__start__而不是START
    for agent in agents:
        workflow.add_edge("supervisor", agent.name)
        workflow.add_edge(agent.name, "supervisor")
    workflow.add_edge("supervisor", "__end__")  # 使用__end__而不是END
