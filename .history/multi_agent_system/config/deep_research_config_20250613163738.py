...
from open_deep_research.src.open_deep_research.configuration import Configuration, SearchAPI
...
def create_research_config(
    ...
    if search_api:
        try:
            ...
            config["configurable"]["search_api"] = SearchAPI.TAVILY
    ...
    if parallel_tool_calls:
        config["configurable"]["parallel_tool_calls"] = True
    
    return config

def config_to_runnable_config(config: Dict[str, Any]) -> RunnableConfig:
    """
    将配置字典转换为RunnableConfig格式
...
__all__ = ["create_default_config", "create_research_config", "config_to_runnable_config"] 