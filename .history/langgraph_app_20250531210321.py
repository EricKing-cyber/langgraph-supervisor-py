...
def check_completion(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "任务完成" in last_message.content or "task completed" in last_message.content.lower():
        return {"next": "__end__"}
    return {"next": "supervisor_communication"}
...
app.add_node("check_completion", check_completion)

# 设置边
app.add_edge("__start__", "task_router")
app.add_edge("task_router", "supervisor")
app.add_edge("supervisor", "check_completion")
app.add_edge("check_completion", "supervisor_communication")
app.add_edge("supervisor_communication", "check_completion")
